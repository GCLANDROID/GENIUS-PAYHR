...
import com.genius.payhrms.activity.attendance.cflo.wfh.WFHApplicationActivity;
...
public class AttendanceCalenderDashboardActivity extends AppCompatActivity implements View.OnClickListener, OnNavigationButtonClickedListener {
    ...
    LinearLayout llCFLO, llCFLOTour, llCFLOWFH, llAllApplicationDetails, llAllApplicationApproval;
    ...
    private void initView() {
        ...
        llCFLO = findViewById(R.id.llCFLO);
        if (pref.getSecurityCode().equalsIgnoreCase("9851")) {
            llCFLO.setVisibility(View.VISIBLE);
            JSONObject object1=new JSONObject();
            try {
                object1.put("CompanyID",pref.getEmpClintId());
                object1.put("SecurityCode",pref.getSecurityCode());
                getMenuForCFLOApplication(object1);
            } catch (JSONException e) {
                e.printStackTrace();
            }

        } else {
            llCFLO.setVisibility(View.GONE);
        }
        llCFLOTour = findViewById(R.id.llCFLOTour);
        llCFLOTour.setOnClickListener(this);

        llCFLOWFH = findViewById(R.id.llCFLOWFH);
        llCFLOWFH.setOnClickListener(this);

        llAllApplicationDetails = findViewById(R.id.llAllApplicationDetails);
        llAllApplicationDetails.setOnClickListener(this);

        llAllApplicationApproval = findViewById(R.id.llAllApplicationApproval);
        llAllApplicationApproval.setOnClickListener(this);
    ...
    public void onClick(View view) {
        if (view == imgMenu) {
            ...
            startActivity(intent);
        } else if (view == llCFLOTour) {
            isAppMinimizeAttendance = true;
            Intent intent = new Intent(AttendanceCalenderDashboardActivity.this, TourApplicationActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
            startActivity(intent);
        } else if (view == llCFLOWFH) {
            isAppMinimizeAttendance = true;
            Intent intent = new Intent(AttendanceCalenderDashboardActivity.this, WFHApplicationActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
            startActivity(intent);
        } else if (view == llAllApplicationDetails) {
            isAppMinimizeAttendance = true;
            Intent intent = new Intent(AttendanceCalenderDashboardActivity.this, OtherApplicationDetailsActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
            startActivity(intent);
        } else if (view == llAllApplicationApproval) {
            isAppMinimizeAttendance = true;
            Intent intent = new Intent(AttendanceCalenderDashboardActivity.this, OtherApplicationApprovalActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
            startActivity(intent);
        }
    }
...
*//*
    ...
    private void getMenuForCFLOApplication(JSONObject object) {

        ProgressDialog pd = new ProgressDialog(AttendanceCalenderDashboardActivity.this);
        pd.setMessage("Loading");
        pd.setCancelable(false);
        pd.show();
        AndroidNetworking.post(Api.CFLOApplicationMenu)
                        ...
                        if (Response_Code.equals("101")) {
                            ...
                            try {
                                ...
                                for( int i=0;i<Table.length();i++){
                                    JSONObject obj=Table.optJSONObject(i);
                                    int MenuID=obj.optInt("MenuID");
                                    if (MenuID==1){
                                        llCFLOTour.setVisibility(View.VISIBLE);
                                    }else if (MenuID==2) {
                                        llCFLOWFH.setVisibility(View.VISIBLE);
                                    }else {
